...
export function createClientTools(ctx: ToolContext) {
  return {
    list_clients: tool({
      ...
      execute: async () => toJsonSafe(await listClients(ctx.organizationId)),
    }),
    search_clients: tool({
      description:
      ...
      execute: async ({ query }) => toJsonSafe(await searchClients(ctx.organizationId, query)),
    }),
    get_client: tool({
      ...
      execute: async ({ clientId }) => {
        ...
        return toJsonSafe(client ?? { error: "not_found" as const });
      },
    }),
    list_client_services: tool({
      description: "List the services/templates a specific client is assigned to.",
      inputSchema: z.object({ clientId: z.string().uuid() }),
      execute: async ({ clientId }) => toJsonSafe(await listClientServices(ctx.organizationId, clientId)),
    }),
  };
}
