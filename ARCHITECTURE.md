...
| Real WhatsApp Integration — M-WA-2 (Real Per-Org Connect) | WhatsApp's "Connect" button (Step3Connect, onboarding step 5) is real for the first time, via Meta Embedded Signup rather than an OAuth redirect — the flow stays inside a popup the whole time (`WhatsAppConnectButton.tsx` loads the Facebook JS SDK and calls `FB.login()` with the app's Embedded Signup Configuration). `FB.login()` returns a short-lived authorization `code`, posted to `POST /api/auth/whatsapp/callback` (JSON, not a redirect). **WA-EMBEDDED_SIGNUP postMessage is not relied on** (confirmed not to fire for this app/config); WABA id is derived server-side via `debug_token` granular scopes, and the phone number via Graph `/phone_numbers`. The exchanged user token is used transiently for immediate post-signup discovery (and best-effort webhook subscribe), then discarded — ongoing send/receive still uses the shared System User token. Connection is stored before webhook subscribe is required to succeed (subscribe is best-effort so a share lag does not block "connected"). Optional `WHATSAPP_OAUTH_REDIRECT_URI` covers Meta error_subcode 36008. Failures return a safe `step` id for diagnostics. See `docs/whatsapp-connect-implementation.md`. |
